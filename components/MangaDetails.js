...
const MangaDetails = ({ mangaId, mangaDetails, mangaStats }) => {
  return (
    <View style={styles.container}>
      ...
      <View style={styles.detailsContainer}>
        ...
        <Text style={styles.author}>{mangaDetails.relationships.filter(rel => rel.type === 'author').map(author => author.attributes.name).join(', ')}</Text>
        <Text style={styles.genres}>{mangaDetails.attributes.tags.filter(tag => tag.attributes.group === 'genre').map(tag => tag.attributes.name.en).join(', ')}</Text>
